...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 1) {
        ...
        if (indexPath.row == 3) {
            ...
            [[[UIAlertView alloc] initWithTitle:@"🚲关于本软件🚲" message:[NSString stringWithFormat:@"CAPUBBS iOS客户端\n版本：%@\n更新时间：%@\n\n原作：熊典|I2\n协助开发：陈章|维茨C\n更新与维护：范志康|好男人\n\n%@\n\n%@", app_Version, UPDATE_TIME, COPYRIGHT, EULA] delegate:nil cancelButtonTitle:@"好" otherButtonTitles: nil] show];
        }
    }
}
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    ...
    if ([segue.identifier isEqualToString:@"web"]) {
        ...
        if (indexPath.row == 0) {
            dest.URL = [NSString stringWithFormat:@"http://%@/bbs", CHEXIE];
        }else if (indexPath.row == 1) {
            dest.URL = [NSString stringWithFormat:@"http://%@", CHEXIE];
        }
    }
